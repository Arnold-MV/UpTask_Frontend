...
const TaskCard = ({ task, canEdit }: TaskCardProps) => {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(data);
      queyClient.invalidateQueries({ queryKey: ["project", projectId] });
    },
  });
  ...
  return (
    <li className="flex justify-between gap-3 p-5 border border-slate-300">
      <div
        {...listeners}
        {...attributes}
        ref={setNodeRef}
      ...
      >
        <p className="text-xl font-bold text-left cursor-pointer text-slate-600">
          {task.name}
        </p>
